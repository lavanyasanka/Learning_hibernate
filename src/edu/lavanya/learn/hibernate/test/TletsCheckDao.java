...
public class TletsCheckDao {
	...
	public void fetchTletsCheck(){
		try {
			...
			session = TletsCheckDao.getSession();
			trans = session.beginTransaction();
			TletsCheck tcheck = session.load(TletsCheck.class, 30655);
			System.out.println("Record to be fatched is : " + session.getEntityName(tcheck));
			trans.commit();
			//System.out.println("Record updated successfully " +session.load(TletsCheck.class, 30655));
			session.close();
			sessionfactory.close();
		} 
		catch (Exception e) {
